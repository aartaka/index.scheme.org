(((name . "string-cursor?")
 ...
 ((group
    ((name . "string-every")
     (signature
       case-lambda
       (((predicate pred) (string? s)) *)
       (((predicate pred) (string? s) ((or integer? string-cursor?) start)) *)
       (((predicate pred)
         (string? s)
         ((or integer? string-cursor?) start)
         ((or integer? string-cursor?) end))
...
Note that the order of arguments is not the same as SRFI 1's list-tabulate, but is the same as tabulation functions in other SRFIs. When this discrepancy was discovered in SRFI 13, it was too late to change SRFI 1."))
 ((name . "string-unfold")
  (signature
   case-lambda
   (((predicate stop?) (procedure? mapper) seed) string?)
   (((predicate stop?) (procedure? mapper) seed (string? base)) string?)
   (((predicate stop?)
     (procedure? mapper)
     seed
     (string? base)
 ...
 ((group
    ((name . "string-trim")
     (signature
       case-lambda
       (((string? s)) string?)
       (((string? s) (predicate pred)) string?)
       (((string? s) (predicate pred) ((or integer? string-cursor?) start))
        string?)
       (((string? s)
         (predicate pred)
         ((or integer? string-cursor?) start)
...
The optional start/end cursors or indexes restrict the comparison to the indicated substrings of s1 and s2."))
 ((group
    ((name . "string-index")
     (signature
       case-lambda
       (((string? s) (predicate pred)) string-cursor?)
       (((string? s) (predicate pred) ((or integer? string-cursor?) start))
        string-cursor?)
       (((string? s)
         (predicate pred)
...
It is an error if start=end — although this is allowed by special dispensation when from=to."))
 ((name . "string-count")
  (signature
   case-lambda
   (((string? s) (predicate pred)) integer?)
   (((string? s) (predicate pred) ((or integer? string-cursor?) start))
    integer?)
   (((string? s)
     (predicate pred)
...
Use SRFI 115's regexp-split to split on a regular expression rather than a simple string."))
 ((group
    ((name . "string-filter")
     (signature
       case-lambda
       (((predicate pred) (string? s)) string?)
       (((predicate pred) (string? s) ((or integer? string-cursor?) start))
        string?)
       (((predicate pred)
         (string? s)
